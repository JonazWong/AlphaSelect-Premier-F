...
docker compose exec -T postgres psql -U postgres -d defaultdb -c "SELECT symbol, COUNT(*) as count FROM contract_markets GROUP BY symbol ORDER BY count DESC LIMIT 10;"
echo.
echo ========================================
echo.
echo 建议：
