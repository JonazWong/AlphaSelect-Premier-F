...
def get_redis_url_with_ssl():
    ...
    if redis_url.startswith('rediss://'):
        ...
        if 'ssl_cert_reqs' not in query_params:
            raw_ssl_cert_reqs = os.getenv('REDIS_SSL_CERT_REQS', 'CERT_NONE').strip()
            normalized_ssl_cert_reqs = raw_ssl_cert_reqs.upper()

            # Be tolerant to common short values to avoid crashing worker startup.
            aliases = {
                'NONE': 'CERT_NONE',
                'OPTIONAL': 'CERT_OPTIONAL',
                'REQUIRED': 'CERT_REQUIRED',
            }
            normalized_ssl_cert_reqs = aliases.get(normalized_ssl_cert_reqs, normalized_ssl_cert_reqs)
            allowed_ssl_cert_reqs = {'CERT_NONE', 'CERT_OPTIONAL', 'CERT_REQUIRED'}

            if normalized_ssl_cert_reqs not in allowed_ssl_cert_reqs:
                normalized_ssl_cert_reqs = 'CERT_NONE'
