...
echo [1] 檢查資料庫中的訓練數據...
docker compose exec -T postgres psql -U postgres -d alphaselect -c "SELECT symbol, COUNT(*) as count FROM contract_markets GROUP BY symbol ORDER BY count DESC;"
echo.

echo [2] 檢查是否有已訓練的模型...
docker compose exec -T postgres psql -U postgres -d alphaselect -c "SELECT model_type, symbol, status FROM ai_models ORDER BY created_at DESC LIMIT 5;"
echo.
