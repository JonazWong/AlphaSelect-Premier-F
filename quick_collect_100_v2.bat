...
echo 使用: 雙擊執行，或在命令列輸入 quick_collect_100_v2.bat
echo.
...
if %errorlevel% neq 0 (
    echo [ERROR] Backend 未運行，請先執行 start.bat 啟動服務
    pause
    exit /b 1
)
...
if "%http_code%"=="200" (
    set /a collected+=1
    echo [!total!/150] [OK] %symbol% - Collected HTTP 200
...
if "%http_code%"=="429" (
    ...
    if %retry_count% lss 2 (
        set /a retry_count+=1
        set /a retried+=1
        echo [!total!/150] [WAIT] %symbol% - HTTP 429 rate limit, retrying...
        timeout /t 3 /nobreak >nul
        goto retry_collect
    )
    set /a errors+=1
    echo [!total!/150] [FAIL] %symbol% - Failed after retry HTTP %http_code%
    endlocal & set /a errors=%errors% & set /a retried=%retried%
    exit /b 1
)
...
if "%http_code%"=="000" (
    ...
    if %retry_count% lss 1 (
        set /a retry_count+=1
        set /a retried+=1
        echo [!total!/150] [RETRY] %symbol% - Connection failed, retrying...
        timeout /t 2 /nobreak >nul
        goto retry_collect
    )
    set /a errors+=1
    echo [!total!/150] [SKIP] %symbol% - Connection failed, skipped
    endlocal & set /a errors=%errors% & set /a retried=%retried%
    exit /b 1
)
...
REM Other errors
set /a errors+=1
echo [!total!/150] [ERROR] %symbol% - HTTP %http_code% error
