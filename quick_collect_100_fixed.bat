...
FOR /L %%i IN (1,1,30) DO (
    for %%s in (%symbols%) do (
        ...
        set "http_code="
        for /f %%c in ('curl -s -o nul -w "%%{http_code}" http://localhost:8000/api/v1/contract/ticker/%%s 2^>nul') do set http_code=%%c
        if not defined http_code set "http_code=000"

        if "!http_code!"=="200" (
            set /a collected+=1
            echo [!total!/150] [OK] %%s - Collected HTTP 200
            goto next_request
        )
        ...
        if "!http_code!"=="429" (
            ...
            if !retry_count! lss 2 (
                set /a retry_count+=1
                set /a retried+=1
                echo [!total!/150] [WAIT] %%s - HTTP 429 rate limit, retrying...
                timeout /t 3 /nobreak >nul
                goto retry_collect
            )
            set /a errors+=1
            echo [!total!/150] [FAIL] %%s - Failed after retry HTTP !http_code!
            goto next_request
        )
        ...
        if "!http_code!"=="000" (
            ...
            if !retry_count! lss 1 (
                set /a retry_count+=1
                set /a retried+=1
                echo [!total!/150] [RETRY] %%s - Connection failed, retrying...
                timeout /t 2 /nobreak >nul
                goto retry_collect
            )
            set /a errors+=1
            echo [!total!/150] [SKIP] %%s - Connection failed, skipped
            goto next_request
        )
        
        REM Other errors
        set /a errors+=1
        echo [!total!/150] [ERROR] %%s - HTTP !http_code! write failed
